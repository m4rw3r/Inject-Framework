<?php
/*
 * Created by Martin Wernståhl on 2009-05-06.
...
function generate($dir)
{
	...
	while(($file = readdir($dh)) !== false)
	{
		...
		$parts = preg_replace('/\/[\d]+/', '/', str_replace(array('./manual', '.html'), '', $dir.'/'.$file));
		
		$parts = explode('/', trim($parts, ' /'));
		
		$p = get_data($dir.'/'.$file);

		$p->nav = nav($parts, count($parts) - 1);

		$p->content = parse($p->content, count($parts) - 1);
		
		$p->base = str_repeat('../', count($parts) - 1);
		...
		file_put_contents('./generated/'.$dir.'/'.end($parts).'.html', $data);
		
		echo "Generated '$dir/".end($parts).".html'<br />";
	}
}
...
function nav($uri, $depth, $base = '')
{
	...
	foreach($main as $part)
	{
		...
		$url = str_repeat('../', $depth) . $link . '.html';
		
		if($base.$current == $link)
		{
			// highlight
			$str .= '<li><a href="'.$url.'" class="current">'.get_data($part)->title.'</a>';
			
			// deeper?
			if(is_dir('./manual/'.$link))
			{
				$d = nav($uri, $depth, $link);
		...
		{
			$str .= '<li><a href="'.$url.'">'.get_data($part)->title.'</a>';
		}
...
function parse($str, $depth)
{
	...
	while(strpos($str, '{link:') && preg_match('@([\w\W]*?)\{link:([^}|]+)(?:\|)?([^}]*)\}([\W\w]*)@', $str, $match))
	{
		...
		{
			$replacement = '<a href="'.str_repeat('../', $depth).$match[2].'" title="'.$file->title.'">'.(( ! empty($match[3])) ? $match[3] : $file->title).'</a>';
		}
